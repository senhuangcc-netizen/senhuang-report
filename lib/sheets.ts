...
// ── 試算表一次性清理（移除群組與標題行，還原純資料）────────────────

async function cleanupSheet(sheetTitle: string) {
  const sheetId = await ensureSheet(sheetTitle)
  ...
  const output = [header, ...dataRows.map(r => { const p = [...r]; while (p.length < ncols) p.push(''); return p })]
...
export async function cleanupAllSheets() {
  if (!process.env.GOOGLE_SERVICE_ACCOUNT_KEY || !process.env.GOOGLE_SHEET_ID) return
  const names = await getSheetNames()
  const reportTab = names[0] ?? '報告總表'
  await cleanupSheet(reportTab)
  await cleanupSheet('X光總表')
  await cleanupSheet('客戶總表')
}
