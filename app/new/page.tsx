...
export default function NewIntakePage() {
  ...
  // 編碼：拆成四段
  const [itemPrefix,   setItemPrefix]   = useState('C26')   // 前綴，預設 C26
  const [itemBracket,  setItemBracket]  = useState('')       // 類型碼，從下拉 [bracket] 自動生成
  const [itemSequence, setItemSequence] = useState('')       // 序號，手動填寫

  // 月份+日期：由建檔當下自動生成（不隨送檢日變動）
